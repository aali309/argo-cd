...
func TestSetOperationState_HelmProvenanceError_PatchesSyncAndConditions(t *testing.T) {
	defaultProj := v1alpha1.AppProject{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "default",
			Namespace: test.FakeArgoCDNamespace,
	...
	app.Spec.Source.RepoURL = "https://prometheus-community.github.io/helm-charts"
	app.Spec.Source.Chart = "alertmanager"
	app.Spec.Source.TargetRevision = "1.33.1"
	app.Spec.Source.Helm = &v1alpha1.ApplicationSourceHelm{}
	// Add ComparisonError so setOperationState includes sync + conditions
	now := metav1.Now()
	app.Status.Conditions = []v1alpha1.ApplicationCondition{{
		Type:               v1alpha1.ApplicationConditionComparisonError,
		Message:            "signed with unallowed key",
	...
	ctrl.setOperationState(app, &v1alpha1.OperationState{Phase: synccommon.OperationError})

	require.NotNil(t, receivedPatch)
	status, ok := receivedPatch["status"].(map[string]any)
	require.True(t, ok, "patch must have status")
	sync, ok := status["sync"].(map[string]any)
	require.True(t, ok, "patch status must have sync for Helm provenance error")
	assert.Equal(t, string(v1alpha1.SyncStatusCodeOutOfSync), sync["status"], "sync.status must be OutOfSync")
	conditions, ok := status["conditions"].([]any)
	require.True(t, ok, "patch status must have conditions")
	assert.GreaterOrEqual(t, len(conditions), 1, "conditions must include ComparisonError")
}

func TestSetOperationState_NonHelmProvenance_NoSyncOrConditionsPatch(t *testing.T) {
	defaultProj := v1alpha1.AppProject{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "default",
			Namespace: test.FakeArgoCDNamespace,
	...
	status, ok := receivedPatch["status"].(map[string]any)
	require.True(t, ok)
	// Verify Helm provenance logic does not apply to Git apps (no sync/conditions patch)
	_, hasSync := status["sync"]
	assert.False(t, hasSync, "Git app should not get sync patch for OperationError")
	_, hasConditions := status["conditions"]
	assert.False(t, hasConditions, "Git app should not get conditions patch for OperationError")
}
